...
public class SqliteProviderTests : IClassFixture<FinanceApiFactory>, IAsyncLifetime
{
    ...
    {
        if (!TestDataSeeder.IsSqlite) return;
    ...
    public void Sqlite_CascadeDelete_RemovesTransactionsWhenUserDeleted()
    {
        ...
        conn.Execute(@"
            ...
            new { H = hash });
        var tempUserId = conn.ExecuteScalar<int>("SELECT Id FROM Users WHERE Username = 'cascade_test'");
        ...
        var countBefore = conn.ExecuteScalar<int>(
        ...
        var countAfter = conn.ExecuteScalar<int>(
            "SELECT COUNT(*) FROM Transactions WHERE UserId = @U", new { U = tempUserId });
